'''
все методы взаимодействия с REST API в нашей будущей библиотеке api.py.
'''
...
class PetFriends:
    ...
    def create_pet_simple(self, auth_key: str, name: str, animal_type: str, age: str) -> json:
        """Метод позволяет создать питомца без фото"""
    ...
    def set_photo(self, auth_key: str, pet_id: str, pet_photo: str) -> json:
        """Метод позволяет добавить фото к созданному питомцу по id"""
        data = MultipartEncoder(
            fields={
                'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')
            })
        headers = {'auth_key': auth_key['key'], 'Content-Type': data.content_type}
        res = requests.post(self.base_url + 'api/pets/set_photo/' + str(pet_id), headers=headers, data=data)
        status = res.status_code
        result = ""
        try:
            result = res.json()
